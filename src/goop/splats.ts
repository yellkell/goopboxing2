 ...
 * Vendored from the GOOP lineage with one change: COLOUR. Two fighters
 * bleed in two colours, so each fighter owns a GooFx tinted to their body
 * (goo that leaves you is YOUR goo, wherever it lands).
 *
 ...
 *  - FLASHES: camera-facing impact/block pops.
 *
...
export class GooFx {
  ...
  /** `tint` is the body colour this mess sprays in (GOOPS.tints[].splat). */
  constructor(tint = 0x54d664) {
    ...
    // clip volume so it draws NOTHING. Living points fade with their alpha
  ...
  /** A burst of light at a world point — a fist connecting, or a glove
   *  stopping one. `color` is 0xRRGGBB, `size` the world radius. */
  ...
  /** Tear the pooled mess down (a rematch builds fresh fx per bout). */
  dispose(): void {
